...
const makeCyclotron = (env = {}, config = {}) => {
  ...
  const makeItConnectable = service => {
    ...
    const connectListeners = [] // TODO dev only

    service._refCount = 0

    service.attach = onDispose => {
      const conn = createConnection()
      service._refCount++
      onDispose(service._disconnect)
      if (connectListeners.length > 0) {
        const detached = onDispose !== onDisposal
        connectListeners.forEach(callback => callback(detached))
      }
      return conn
    }

    service.connect = () => service.attach(onDisposal)
    ...
    service.onConnect = callback => connectListeners.push(callback)

    return service
  }

  const flushOuroboros = service => {
    const ouroborosCallbacks = resolvingHandlers.get(service.handler)
