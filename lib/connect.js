...
export const createConnector = env => {
  ...
  connect.sink = factory => {
    const result = connect(factory)
    if (!result) {
    ...
    throw new Error('connect.sink cannot resolve read-only providers')
  }

  connect.source = factory => {
    const result = connect(factory)
    if (!result) {
      throw new Error('Illegal state')
    }
    if (typeof result === 'function') {
      throw new Error('connect.source cannot resolve write-only providers')
    }
    if (result.subscribe) {
      return result
    }
